...
We first construct the signal $\beta$ that is piecewise constant on an 8 by 8 grid graph, with three true piecewise constant segments which take on values $-3$, $0$, and $3$. We will also use the `getD2d` function from the `genlasso` package to create the penalty matrix $D$ that corresponds to this grid graph.
...
```
The code below visualizes the signal $\beta$. For convenience, we will refer to the three segments of $\beta$ as $C_1$, $C_2$, and $C_3$ (colored in magenta, green, and grey in the figure below).
```{r fig.align="center",  warning = FALSE, fig.height = 5, fig.width = 4}
...
The code below displays the observation `y`.
```{r fig.align="center",  fig.height = 5, fig.width = 4, warning = FALSE}
...
In the code below, we use the `fusedlasso` function (see [link](https://rdrr.io/cran/genlasso/man/fusedlasso.html) for the detailed documentations for the `fusedlasso` function) in the `genlasso` package to obtain the graph fused lasso estimator $\hat\beta$ on data `y`. In this example, we apply the dual path algorithm on `y` with $K=13$, which yields three estimated connected components. 
```{r fig.align="center",  fig.height = 5, fig.width = 4, warning = FALSE}
K = 13
...
We now have the the difference in means, i.e., $(\sum_{j \in \hat{C}_1} y_j)/|\hat{C}_1| -(\sum_{j \in \hat{C}_3} y_j)/|\hat{C}_3|$ (`test_stats`), $p_{Hyun}$ (`pval_hyun`), $p_{\hat{C}_1,\hat{C}_2}$ (`pval_c1c2`), and accompanying $(1-\alpha)$ confidence intervals (`[LCB,UCB]`). Because $p_{Hyun}$ conditions on too much information, the test based on  $p_{Hyun}$ has low power cannot reject $H_0$ at level $\alpha=0.05$ ($p_{Hyun}=0.25$). By contrast, the test based on $p_{\hat{C}_1,\hat{C}_2}$ can easily this null hypothesis ($p_{\hat{C}_1,\hat{C}_2}<0.001$).
